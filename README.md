...
Напишите функцию `emailAnalyzer`, которая принимает строку и возвращает объект с анализом электронных адресов, содержащихся в этой строке. Объект должен содержать следующие свойства:

* emailCount: количество электронных адресов в строке.
* uniqueEmails: массив уникальных электронных адресов, найденных в строке.
* mostFrequentEmail: электронный адрес, который встречается чаще всего (если несколько адресов имеют одинаковую частоту, вернуть любой из них).

Электронные адреса определяются как последовательности символов, содержащие символы до символа @, за которым следует домен, который может содержать буквы, цифры и точки.

Функция должна игнорировать регистр при подсчете уникальных адресов и частоты.
...
- `test/emailAnalyzer.js` &mdash; файл с тестами к функции `emailAnalyzer()`
