# frontend-homework, вариант №1
> Introductory homework for the course "Frontend-development"

## Привет!

В этом репозитории содержится условие варианта №1. Чтобы начать с ним работать, тебе необходимо склонировать репозиторий к себе на компьютер и перейти в ветку твоего варианта.
...
Напишите функцию `fetchAndMerge`, которая будет принимать массив URL-адресов, загружать данные с этих адресов (например, JSON), и возвращать объединенный объект, содержащий все уникальные ключи из загруженных данных. 

Если один и тот же ключ встречается в нескольких объектах, его значение должно быть массивом, содержащим все уникальные значения для этого ключа.
...
- `test/fetchAndMerge.js` &mdash; файл с тестами к функции `fetchAndMerge()`
