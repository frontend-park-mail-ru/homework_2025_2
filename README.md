...
Напишите функцию `filterByKeys`, которая принимает объект `obj` и массив ключей `keys`, и возвращает новый объект, содержащий только те ключи из исходного объекта, которые указаны в массиве `keys`. 

Если ключ из массива `keys` отсутствует в объекте, он должен игнорироваться. При этом, если значение по ключу является объектом, оно должно быть скопировано в результирующий объект, но не должно быть изменено.
...
- `test/filterByKeys.js` &mdash; файл с тестами к функции `filterByKeys()`
