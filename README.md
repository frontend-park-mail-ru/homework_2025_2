...
Напишите функцию `transform`, которая принимает объект `obj` и функцию преобразования `transformFn`. 

Функция `transformFn` должна применяться к каждому значению в объекте, и результирующий объект должен содержать те же ключи, но с преобразованными значениями. Если значение является объектом, оно должно быть обработано рекурсивно.
...
- `test/transform.js` &mdash; файл с тестами к функции `transform()`
