# frontend-homework, вариант №10
> Introductory homework for the course "Frontend-development"

## Привет!

В этом репозитории содержится условие варианта №10. Чтобы начать с ним работать, тебе необходимо склонировать репозиторий к себе на компьютер и перейти в ветку твоего варианта.
...
Напишите функцию `mergeBy`, которая принимает два массива объектов и строку, представляющую ключ. Функция должна объединять объекты из обоих массивов по указанному ключу. 

Если объекты имеют одинаковое значение по этому ключу, они должны быть объединены в один объект, содержащий все свойства из обоих объектов. 

Если одно из свойств отсутствует в одном из объектов, оно должно быть добавлено из другого объекта. При этом, если одно из свойств является массивом, они должны быть объединены в один массив без дубликатов.
...
- `test/mergeBy.js` &mdash; файл с тестами к функции `mergeBy()`
