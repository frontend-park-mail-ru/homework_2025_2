...
QUnit.module('Тестируем функцию factorial', () => {
    ...
    QUnit.test('Факториал для объекта должен выбрасывать ошибку', (assert) => {
        assert.throws(() => {
            factorial({key: 'value'});
        }, /Факториал определен только для целых неотрицательных чисел/, 'Ошибка выбрасывается для объекта');
    });

    QUnit.test('Факториал для undefined должен выбрасывать ошибку', (assert) => {
    assert.throws(() => {
        factorial(undefined);
    }, /Факториал определен только для целых неотрицательных чисел/, 'Ошибка для undefined');
});
});
